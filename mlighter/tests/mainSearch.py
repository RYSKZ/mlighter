...
session = MLighter()
session.uploadDataset(
    "structured", datasetName="iris", actualData=iris.data, targetData=iris.target
)
# print(session.data.getColumns())
session.uploadModel(
    "sklearn", "iris", modelUrl=homeTool + "/tests/example/irisProb.joblib"
)
